...
import { createNewIncident,isIncidentExists } from "../../../helper/servicenowApiUtils"
...
test.describe(`Incident creation`,async()=>{
    ...
    test(`TC002 - Create a new incident by API on service now instance`,async({page})=>{
        const incidentNum = await createNewIncident(page.request,"Creating by APIs")
        const isIncExist = await isIncidentExists(page.request,incidentNum)
        expect(isIncExist).toBe(true)
